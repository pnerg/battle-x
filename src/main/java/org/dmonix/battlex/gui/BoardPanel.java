...
public class BoardPanel extends JPanel implements GameEventListener
{
  ...
  /**
   * Removes all the players pieces.
   * <br>It will also notify the other player of these changes
   * This is only used during setup when a player loads a setup from file
   */
  ...
  public void gameEvent(GameEventObject geo)
  {
    ...
    if (gameStateObject.inState(GameStateController.STATE_IN_GAME_OPPONENT_TURN))
    {
      ...
      movePiece(pieces[geo.getOldXCoord()][geo.getOldYCoord()],
                       geo.getNewXCoord(),
                       geo.getNewYCoord());
    }
    /**
     ...
     */
    else if(gameStateObject.inState(GameStateController.STATE_GAME_SETUP) ||
            gameStateObject.inState(GameStateController.STATE_SETUP_WAIT_OPPONENT_SETUP))
    {
      if(geo.getType() != Resources.PIECE_NO_PIECE)
        this.pieces[geo.getNewXCoord()][geo.getNewYCoord()] = new Piece(otherPlayer,
                                                                        geo.getType(),
                                                                        geo.getNewXCoord(),
                                                                        geo.getNewYCoord());
      else
   ...
   * @param eventCommunicator The communicator
   */
  ...
  public void paint(Graphics g)
  {
    ...
    /**
     * pre-load all images (for both players) the first time, otherwise it might sometimes
     * result in strange behaviour where the image isn't rendered until after
     * a few repaints
     */
    if(firstPaint)
    {
...
//    System.out.println("Boardpanel w="+dim.width+"; h="+dim.height+"   #   sw="+squareWidth+": sh="+squareHeight);
       ...
       (currentPiece == null && gameStateObject.inState(GameStateController.STATE_GAME_SETUP_RECEIVED_SETUP)))
    {
      ...
      for (int y= 0; y < 10; y++)
      {
        for (int x= 0; x < 10; x++)
        {
          ...
          {
            g2.setColor(fill);
            g2.fill(new Rectangle((int)(squareWidth * x), (int)(squareHeight * y),
  ...
  void inGameClick(MouseEvent e)
  {
    ...
    /**
     * If the user has pressed any other mouse button than button 1
    ...
    /**
     * If a piece has been selected it's
     * only valid to move to highlighted squares
     */
    if (currentPiece != null && markedSquares[point.x][point.y] == null)
      return;

    eventCommunicator.sendEvent(new GameEventObject(currentPiece.getLocation().x,
                                                    currentPiece.getLocation().y,
                                                    point.x,
                                                    point.y));
   ...
   * @param point The point for the piece
  ...
  private void movePiece(Piece attacker, int x_coord_defender, int y_coord_defender)
  {
            ...
            defender.getPlayer() != attacker.getPlayer())
    {
      ...
      if(result == Piece.RESULT_WIN)
      {
        ...
        ResolveStrikeDialog.showStrikeResult(owner, "Player "+attacker.getPlayer()+" wins",
                                             attacker.getPlayer(), attacker.getType(),
      ...
      else if(result == Piece.RESULT_DRAW)
      {
        ...
        ResolveStrikeDialog.showStrikeResult(owner, "Draw",
                                             attacker.getPlayer(), attacker.getType(),
                                             defender.getPlayer(), defender.getType());
      }
      //defender wins
      else if(result == Piece.RESULT_LOOSE)
      {
        ...
        ResolveStrikeDialog.showStrikeResult(owner, "Player "+defender.getPlayer()+" wins",
                                             attacker.getPlayer(), attacker.getType(),
                                             defender.getPlayer(), defender.getType());
      ...
      {
        ResolveStrikeDialog.showStrikeResult(owner, "Game over! Player"+attacker.getPlayer()+" wins",
                                             attacker.getPlayer(), attacker.getType(),
                                             defender.getPlayer(), defender.getType());
  ...
  /**
   * Checks if any of the players can move a piece.
   * If not a message it displayed stating the victory of one of the players
   */
  ...
  private void markSquares(Piece piece)
  {
    ...
    {
      if(y_coord-i < 0 ||
         ((x_coord == 2 || x_coord == 3 || x_coord == 6 || x_coord == 7) && (y_coord-i == 4 || y_coord-i == 5)) ||
    ...
    {
      if(x_coord+i > 9 ||
         ((x_coord+i == 2 || x_coord+i == 3 || x_coord+i == 6 || x_coord+i == 7) && (y_coord == 4 || y_coord == 5)) ||
  ...
  private class BoardPanel_this_mouseAdapter extends java.awt.event.MouseAdapter
  {
    ...
    public void mouseClicked(MouseEvent e)
    {
      ...
      if(gameStateObject.inState(GameStateController.STATE_GAME_SETUP) ||
         gameStateObject.inState(GameStateController.STATE_GAME_SETUP_RECEIVED_SETUP))
